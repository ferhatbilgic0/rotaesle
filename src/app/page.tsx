  ...
  return (
    <div className="stat-card group">
      <div className="flex items-start justify-between mb-4">
        <div className={cn("w-11 h-11 rounded-2xl flex items-center justify-center", c.ring)}>
      ...
      <div className="flex">
        {[1, 2, 3, 4, 5].map((i) => (
          <svg
            ...
            className={cn(
              "w-3 h-3",
              i <= Math.floor(score) ? "text-amber-400" : "text-slate-200"
            )}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
...
function RouteVisual({ from, to }: { from: string; to: string }) {
  return (
    <div className="flex items-center gap-1.5 text-sm">
      ...
      <div className="flex-1 flex items-center gap-0.5 min-w-[24px]">
...
export default function DashboardPage() {
  ...
  return (
    <div className="page-enter p-8 space-y-8 max-w-[1400px]">
      {/* Header */}
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            ...
            <span className="font-semibold text-blue-600">
              {stats.totalActiveLoads} aktif yük
            </span>{" "}
            rotanızı bekliyor. Son güncelleme: az önce.
          </p>
        </div>
        <Link href="/eslestir">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-5">
        <StatCard
          title="Aktif Yük"
          value={String(stats.totalActiveLoads)}
          subtitle="Hemen alınabilir"
          icon={Package}
          trend="+8%"
          color="blue"
        />
        <StatCard
          ...
          subtitle="Yolda veya yükleniyor"
          icon={TruckIcon}
          trend="+3%"
          color="amber"
        />
        <StatCard
          title="Önlenen CO₂"
          value={`${(stats.preventedEmissionsKg / 1000).toFixed(1)}t`}
        ...
        />
        <StatCard
          title="Platform Cirosu"
          value={formatTL(stats.totalRevenueTL)}
          subtitle="Tamamlanan yükler"
          icon={TrendingUp}
          trend="+15%"
          color="purple"
        />
      ...
      <div className="card-base p-5">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-slate-800">Yük Durumu Özeti</h2>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: "Teslim", value: stats.totalLoads - stats.totalActiveLoads - stats.matchedLoads - stats.inTransitLoads, color: "bg-slate-300", pct: ((stats.totalLoads - stats.totalActiveLoads - stats.matchedLoads - stats.inTransitLoads) / stats.totalLoads) * 100 },
          ].map((item) => (
            <div key={item.label}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card-base overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {recentLoads.map((load) => (
              <div key={load.id} className="px-6 py-3.5 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-slate-900">
                      {formatTL(load.recommendedPriceTL)}
                    </p>
        ...
        <div className="space-y-5">
          ...
          {/* CTA Card */}
          <div className="relative overflow-hidden rounded-2xl gradient-blue p-6 text-white">
            <div className="relative z-10">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center mb-4">
                <Zap className="w-5 h-5 text-white" />
              </div>
              <h3 className="font-bold text-lg leading-tight mb-2">
                Boş Dönüş?<br />Para Kaybetme.
              </h3>
              <p className="text-sm text-blue-100 mb-4 leading-relaxed">
                Rota üzerindeki yükleri bul, ekstra kazanç elde et. Algoritma senin için çalışıyor.
              </p>
              <Link href="/eslestir">
